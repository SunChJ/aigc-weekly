...
- **备选方案**：如果 Firecrawl 不可用或失败，可降级使用 `WebFetch` 相关工具。
- **多级爬取**：如果目标是一个列表页（如 Hacker News 首页），你需要识别列表中的详情页链接并深入抓取，但深度通常不超过 3 层。如果是转载文章，优先抓取原始来源。
...
1.  **获取与解析 (Fetch & Parse)**:
    - 访问目标 URL。
    - **识别页面类型**：判断是详情页还是列表页。
      - 如果是**列表页**（包含多篇文章链接），提取所有文章详情页链接，然后**逐个抓取详情页内容**。
      - 如果是**详情页**，直接提取内容。
      - 如果是转载文章，优先抓取原始来源。
...
2.  **筛选与验证 (Filter & Verify)**:
    ...
    - **时间有效性**：如果用户指定了时间范围，检查文章的 `published_time`。**不在时间范围内的文章应丢弃**。如果文章未包含明确时间但 URL 或上下文暗示了时间（如 Hacker News 每日列表），则依据上下文判断。

3.  **格式化与保存 (Format & Save)**:
    - **保存完整原文**：将文章的完整正文内容保存为 Markdown 格式，保留原文的段落结构、重要链接和图片。
