...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`weekly_links\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`label\` text NOT NULL,
  	\`url\` text NOT NULL,
  	FOREIGN KEY (\`_parent_id\`) REFERENCES \`weekly\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`weekly_links_order_idx\` ON \`weekly_links\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`weekly_links_parent_id_idx\` ON \`weekly_links\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`weekly_tags\` (
  	\`_order\` integer NOT NULL,
  	\`_parent_id\` integer NOT NULL,
  	\`id\` text PRIMARY KEY NOT NULL,
  	\`value\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`weekly_tags_order_idx\` ON \`weekly_tags\` (\`_order\`);`)
  await db.run(sql`CREATE INDEX \`weekly_tags_parent_id_idx\` ON \`weekly_tags\` (\`_parent_id\`);`)
  await db.run(sql`CREATE TABLE \`weekly\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`title\` text NOT NULL,
  ...
  await db.run(sql`CREATE TABLE \`payload_mcp_api_keys\` (
  	...
  	\`label\` text,
  	\`description\` text,
  	\`weekly_find\` integer DEFAULT false,
  	\`weekly_create\` integer DEFAULT false,
  	\`weekly_update\` integer DEFAULT false,
  	\`weekly_delete\` integer DEFAULT false,
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  ...
  await db.run(sql`CREATE TABLE \`payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`weekly_id\`) REFERENCES \`weekly\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_weekly_id_idx\` ON \`payload_locked_documents_rels\` (\`weekly_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`weekly_links\`;`)
  await db.run(sql`DROP TABLE \`weekly_tags\`;`)
